...
.CP-root {
  ...
  --jade:  #FF6B00; --jade2: #fe9307; --jade3: #ff9f3a;
  --jade-t: rgba(255,107,0,.08); --jade-g: rgba(255,107,0,.18); --jade-r: rgba(255,107,0,.26);
  --gold:  #C2410C; --gold2: #EA580C; --gold3: #FB923C;
  --gold-t: rgba(194,65,12,.08); --gold-g: rgba(194,65,12,.18); --gold-r: rgba(194,65,12,.26);
  --iris:  #9A3412; --iris2: #B0430F;
...
@keyframes CP-glow      { 0%,100%{box-shadow:0 0 0 0 rgba(224,123,26,.25)} 50%{box-shadow:0 0 0 6px rgba(224,123,26,.08)} }
...
/* ── SUCCESS CELEBRATION — ported from Credit Management's post-payment
...
.CP-closed-amt-badge {
  ...
  border: 1.5px solid #fdc87a; box-shadow: 0 2px 12px rgba(203,54,9,0.10);
  border-radius: 100px; font-family: var(--ff-m,'JetBrains Mono',monospace);
  font-size: 16px; font-weight: 800; color: #FF6B00; margin-top: 4px; overflow: hidden;
}
...
/* Page-open runner intro (see PageOpenIntro.tsx). Same mechanism as the
   main Credit Management / Daybook pages: real content is hidden with
...
/* BUTTONS */
.CP-btn-primary { display:inline-flex; align-items:center; justify-content:center; gap:7px; height:34px; padding:0 22px; border-radius:var(--r2); font-family:var(--ff-m); font-size: 9px; font-weight: 800; letter-spacing:1.6px; text-transform:uppercase; white-space:nowrap; cursor:pointer; border:none; background:linear-gradient(135deg,#fe9307 0%,#ff6b00 100%); color:#fff; box-shadow:var(--sh-or); transition:transform .2s,box-shadow .2s,background .2s; }
.CP-btn-primary:hover:not(:disabled) { transform:translateY(-2px); box-shadow:var(--sh-or2); background:linear-gradient(135deg,#ff9f3a 0%,#E8820A 100%); }
.CP-btn-primary:disabled { opacity:.45; cursor:not-allowed; }
.CP-btn-add-compact { display:inline-flex; align-items:center; gap:6px; padding:7px 15px; border-radius:999px; font-family:var(--ff-m); font-size: 8px; font-weight: 800; letter-spacing:1.6px; text-transform:uppercase; white-space:nowrap; cursor:pointer; border:none; background:linear-gradient(135deg,#fe9307 0%,#ff6b00 100%); color:#fff; box-shadow:0 4px 14px rgba(203,54,9,.32); transition:transform .18s ease,box-shadow .18s ease,background .18s ease; }
.CP-btn-add-compact:hover { transform:translateY(-2px) scale(1.03); box-shadow:0 7px 20px rgba(203,54,9,.45); background:linear-gradient(135deg,#ff9f3a 0%,#E8820A 100%); }
...
.CP-btn-header-add {
  ...
  background:linear-gradient(135deg,#fe9307 0%,#ff6b00 100%);
  box-shadow:0 4px 14px rgba(203,54,9,.32), 0 2px 6px rgba(203,54,9,.2);
...
.CP-btn-header-add:hover { transform:translateY(-2px) scale(1.03); box-shadow:0 8px 20px rgba(203,54,9,.42), 0 3px 8px rgba(203,54,9,.28); }
...
.CP-btn-outline:hover { background:linear-gradient(135deg,#fe9307,#ff6b00); border-color:transparent; color:#fff; box-shadow:var(--sh-or); transform:translateY(-2px); }
...
.CP-sb-count-num {
  font-family: var(--font-mono); font-size: 8px; font-weight: 800;
  color: var(--ember,#FF6B00); background: #fff7ed;
  padding: 1px 7px; border-radius: 100px; border: 1px solid #fdc87a;
}

/* Client card list scroll */
.CP-cli-list { flex: 1; overflow-y: auto; padding: 4px 0 8px; }
.CP-cli-list::-webkit-scrollbar { width: 4px; }
.CP-cli-list::-webkit-scrollbar-thumb { background: var(--border,#e8e3d9); border-radius: 2px; }
...
.CP-clicard:hover .CP-clicard-av { transform: scale(1.1) rotate(-3deg); box-shadow: 0 4px 12px rgba(203,54,9,0.18); }
.CP-clicard.active .CP-clicard-av { transform: scale(1.06); box-shadow: 0 4px 14px rgba(203,54,9,0.22); }
...
.CP-clicard-hint {
  ...
  background: rgba(203,54,9,0.07); color: #FF6B00;
...
@keyframes CP-card-attention {
  ...
  50%     { box-shadow: 0 0 0 4px rgba(203,54,9,0.18), 0 4px 16px rgba(203,54,9,0.12); }
}
...
.CP-sb-tap-icon {
  ...
  background: linear-gradient(135deg,rgba(203,54,9,0.12),rgba(254,147,7,0.12));
  border: 1.5px dashed rgba(203,54,9,0.35);
  display: flex; align-items: center; justify-content: center;
  animation: CP-tap-pulse 1.8s ease-in-out infinite;
}
@keyframes CP-tap-pulse {
  0%,100% { box-shadow: 0 0 0 0 rgba(203,54,9,0); }
  50%     { box-shadow: 0 0 0 6px rgba(203,54,9,0.12); }
}
.CP-sb-tap-label { font-family:var(--ff-m); font-size: 8px; letter-spacing:1.5px; text-transform:uppercase; color:var(--ember,#FF6B00); }
...
.CP-main-content { flex: 1; padding: 12px; background: var(--surface,#f8f7f4); overflow-y: auto; }
...
.CP-pjcard:hover { box-shadow: 0 6px 28px rgba(0,0,0,0.12); transform: translateY(-2px); border-color: #fdc87a; }
...
.CP-pjcard-left::before {
  ...
  background: linear-gradient(180deg,#FF6B00,#fe9307);
  box-shadow: 2px 0 8px rgba(203,54,9,0.2);
}
.CP-pjcard-num {
  font-family: var(--font-mono,'JetBrains Mono',monospace);
  font-size: 17.5px; font-weight: 900; color: var(--ember,#FF6B00);
  line-height: 1; letter-spacing: -1px;
}
.CP-pjcard-icon {
  width: 28px; height: 28px; border-radius: 8px;
  background: rgba(203,54,9,0.08); border: 1.5px solid rgba(203,54,9,0.2);
  display: flex; align-items: center; justify-content: center;
...
.CP-pjcard-chip-type { background:#fff7ed; color:#FF6B00; border-color:#fdc87a; }
...
.CP-pjcard-prog-pct { font-family:var(--font-mono); font-size: 8px; font-weight: 800; color:var(--ember,#FF6B00); flex-shrink:0; }
.CP-pjcard-mini-bar { flex:1; height:5px; border-radius:100px; background:#f1f5f9; overflow:hidden; }
.CP-pjcard-mini-fill {
  height:100%; border-radius:100px;
  background:linear-gradient(90deg,#FF6B00,#fe9307);
  transition:width 1.2s cubic-bezier(0.4,0,0.2,1);
  box-shadow: 0 1px 3px rgba(203,54,9,0.3);
}
...
.CP-pjcard-right {
  ...
  border-left: 1px solid var(--border,#e8e3d9);
}
.CP-pjcard-amt {
  font-family: var(--font-mono,'JetBrains Mono',monospace);
...
.CP-pjcard-status.open  { background:#fff7ed; color:#FF6B00; border-color:#fdc87a; }
...
.CP-pjcard-acts {
  ...
  padding: 8px 7px; gap: 5px; border-left: 1px solid var(--border,#e8e3d9);
  background: linear-gradient(180deg,#fafaf9,#f5f3ef);
}
.CP-pjcard-act {
...
.CP-pjcard-act.bgt { background:#fff7ed; border-color:#fdc87a; color:#FF6B00; }
.CP-pjcard-act.bgt:hover { background:#FF6B00; border-color:#FF6B00; color:#fff; box-shadow:0 3px 12px rgba(203,54,9,0.3); transform:translateY(-1px); }
...
.CP-main-section-pill {
  font-family: var(--font-mono); font-size: 8px; font-weight: 800;
  color: var(--ember,#FF6B00); background: #fff7ed;
  padding: 2px 8px; border-radius: 100px; border: 1px solid #fdc87a;
}
...
.CP-main-stats {
  ...
  border-bottom: 1.5px solid var(--border,#e8e3d9);
}
...
.CP-main-stat::before {
  ...
  background: var(--stat-ac, linear-gradient(90deg,#FF6B00,#fe9307));
  border-radius:12px 12px 0 0; transition: height .18s;
}
.CP-main-stat:hover { transform:translateY(-3px); box-shadow:0 6px 20px rgba(0,0,0,0.1); border-color:#fdc87a; }
...
/* SKELETON */
.CP-skel { background:linear-gradient(90deg,var(--off-white,#f5f3ef) 25%,var(--surface-3,#eae8e4) 50%,var(--off-white,#f5f3ef) 75%); background-size:700px 100%; animation:CP-shimmer 1.4s infinite linear; border-radius:8px; }

/* EMPTY / FOOTER */
.CP-empty { display:flex; flex-direction:column; align-items:center; padding:60px 24px; text-align:center; }
.CP-empty-ico { width:54px;height:54px;border-radius:50%;background:#fff7ed;border:1.5px solid #fdc87a;display:flex;align-items:center;justify-content:center;margin:0 auto 16px;opacity:.7; }
...
.CP-detail-prog { background:var(--w,#fff); border:1.5px solid rgba(203,54,9,0.18); border-radius:var(--r2); padding:14px 16px; margin-bottom:16px; }
...
.CP-detail-prog-pct { font-family:var(--ff-m); font-size: 16px; font-weight: 800; color:var(--ember,#FF6B00); letter-spacing:-0.5px; }
.CP-detail-prog-bar { height:7px; background:rgba(203,54,9,0.1); border-radius:99px; overflow:hidden; }
.CP-detail-prog-fill { height:100%; border-radius:99px; background:linear-gradient(90deg,#A82800,#C86A00); animation:CP-barFill .9s .1s cubic-bezier(.34,1.2,.64,1) both; min-width:3px; }
...
.CP-tx-scroll { max-height:320px; overflow-y:auto; scrollbar-width:thin; scrollbar-color:rgba(203,54,9,0.25) transparent; }
.CP-tx-scroll::-webkit-scrollbar { width:5px; }
.CP-tx-scroll::-webkit-scrollbar-thumb { background:rgba(203,54,9,0.22); border-radius:99px; }
.CP-tx-scroll::-webkit-scrollbar-track { background:transparent; }
.CP-tx-feed-hdr { display:flex; align-items:center; justify-content:space-between; padding:10px 14px; background:var(--off-white,#fdf9f3); border-bottom:1.5px solid var(--bd); }
.CP-tx-feed-title { display:flex; align-items:center; gap:7px; font-family:var(--ff-m); font-size: 8px; font-weight: 800; letter-spacing:1.8px; text-transform:uppercase; color:var(--ember,#FF6B00); }
.CP-tx-count-badge { background:rgba(203,54,9,0.1); color:var(--ember,#FF6B00); font-family:var(--ff-m); font-size: 8px; font-weight: 800; padding:2px 9px; border-radius:20px; border:1px solid rgba(203,54,9,0.2); }
/* ERP ledger row */
...
.CP-tx-item:hover { background:rgba(203,54,9,0.03); }
.CP-tx-item:last-child { border-bottom:none; }
.CP-tx-item::before { content:''; position:absolute; left:0; top:6px; bottom:6px; width:2.5px; background:var(--ember,#FF6B00); border-radius:0 2px 2px 0; opacity:0; transition:opacity .15s; }
.CP-tx-item:hover::before { opacity:1; }
/* Icon — neutral monochrome */
.CP-tx-icon-erp { width:34px; height:34px; border-radius:9px; flex-shrink:0; display:flex; align-items:center; justify-content:center; background:rgba(203,54,9,0.07); border:1px solid rgba(203,54,9,0.16); }
...
.CP-tx-mode-badge { display:inline-flex; align-items:center; font-family:var(--ff-m); font-size: 7.5px; font-weight: 800; letter-spacing:.5px; text-transform:uppercase; padding:2px 8px; border-radius:20px; border:1px solid; white-space:nowrap; background:rgba(203,54,9,0.07); color:var(--ember,#FF6B00); border-color:rgba(203,54,9,0.18); }
...
.CP-tx-view-more { display:flex; align-items:center; justify-content:center; gap:8px; padding:10px 14px; background:var(--off-white,#fdf9f3); border-top:1px solid var(--bd); font-family:var(--ff-m); font-size: 8px; font-weight: 800; text-transform:uppercase; letter-spacing:1px; color:var(--ember,#FF6B00); cursor:pointer; transition:all .18s; }
.CP-tx-view-more:hover { background:rgba(203,54,9,0.06); gap:14px; }
.CP-tx-empty { display:flex; flex-direction:column; align-items:center; padding:32px 24px; background:var(--off-white,#fdf9f3); text-align:center; }
.CP-tx-empty-icon { width:44px; height:44px; border-radius:50%; background:rgba(203,54,9,0.07); border:1.5px dashed rgba(203,54,9,0.25); display:flex; align-items:center; justify-content:center; margin-bottom:11px; }
...
.CP-prog-panel {
  ...
  border:1px solid var(--border,#E7D7C1); overflow:hidden;
  background:var(--white,#FFFDFA);
  box-shadow:0 1px 3px rgba(203,54,9,0.05), 0 12px 30px rgba(203,54,9,0.08);
  animation:bc-in 0.3s both;
}
.CP-prog-hdr {
  display:flex; align-items:center; justify-content:space-between; gap:10px;
  padding:13px 16px;
  background:linear-gradient(120deg, var(--off-white,#FDF7EF) 0%, var(--surface,#FAF2E7) 55%, #FBEAD8 100%);
  border-bottom:1px solid rgba(255,107,0,0.14);
  position:relative; overflow:hidden;
}
.CP-prog-hdr::after {
  content:''; position:absolute; left:0; right:0; bottom:0; height:2px;
  background:linear-gradient(90deg,#FF6B00,#FF6B00,#ffb347,transparent);
}
.CP-prog-hdr-left {
  display:flex; align-items:center; gap:8px;
  font-family:var(--ff-m); font-size: 8px; font-weight: 800;
  letter-spacing:1.8px; text-transform:uppercase; color:#6B4A2A;
}
.CP-prog-hdr-icon {
  width:24px; height:24px; border-radius:8px; flex-shrink:0;
  display:flex; align-items:center; justify-content:center;
  background:linear-gradient(135deg,rgba(255,107,0,0.16),rgba(255,107,0,0.06));
...
.CP-prog-hdr-badge.good { background:rgba(255,107,0,0.12); color:#FF6B00; border-color:rgba(255,107,0,0.28); }
.CP-prog-hdr-badge.mid  { background:rgba(196,126,10,0.14); color:#8a5c07; border-color:rgba(196,126,10,0.28); }
...
.CP-prog-body {
  ...
  background:radial-gradient(circle at 0% 0%, rgba(255,107,0,0.045), transparent 55%);
}
.CP-prog-ring-wrap {
...
.CP-prog-ring-wrap::before {
  ...
  background:radial-gradient(circle, rgba(255,107,0,0.12) 0%, transparent 72%);
}
.CP-prog-svg {
  width:136px; height:136px; display:block; position:relative;
  filter:drop-shadow(0 6px 16px rgba(255,107,0,0.24));
}
/* Right side — elevated KPI cards */
.CP-prog-stats { flex:1; display:flex; flex-direction:column; gap:9px; min-width:0; }
.CP-kpi-card {
  position:relative; overflow:hidden;
  background:linear-gradient(135deg,var(--white,#FFFDFA) 0%,var(--off-white,#FDF7EF) 100%);
  border:1px solid var(--border,#E7D7C1); border-radius:13px;
  padding:10px 13px 11px;
  box-shadow:0 1px 2px rgba(203,54,9,0.03);
  transition:transform .18s ease, box-shadow .18s ease, border-color .18s ease;
  animation:bc-in 0.35s both;
}
.CP-kpi-card::before { content:''; position:absolute; top:0; left:0; right:0; height:3px; }
.CP-kpi-card.collected::before { background:linear-gradient(90deg,#FF6B00,#FF6B00,#ffb347); }
.CP-kpi-card.pending::before   { background:linear-gradient(90deg,#D93B55,#f87171); }
.CP-kpi-card.total::before     { background:linear-gradient(90deg,#C86A00,#fe9307); }
.CP-kpi-card:hover { transform:translateY(-2px); box-shadow:0 10px 22px rgba(203,54,9,0.1); border-color:rgba(255,107,0,0.3); }
...
.CP-kpi-icon.collected { background:rgba(255,107,0,0.13); }
.CP-kpi-icon.pending   { background:rgba(220,38,38,0.13); }
.CP-kpi-icon.total     { background:rgba(200,106,0,0.13); }
...
.CP-kpi-bar-track { height:5px; background:rgba(203,54,9,0.07); border-radius:100px; overflow:hidden; margin-bottom:5px; }
...
.CP-pay-block-hdr {
  ...
  background:rgba(203,54,9,0.04); border-bottom:1px solid var(--bd);
  font-family:var(--ff-m); font-size: 8px; font-weight: 800;
  letter-spacing:1.5px; text-transform:uppercase; color:#FF6B00;
}
.CP-pay-block-badge {
...
.CP-pay-row:hover { background:rgba(255,107,0,0.045); }
.CP-pay-mode {
  font-family:var(--ff-m); font-size: 7px; font-weight: 800;
  letter-spacing:0.6px; color:#FF6B00;
  background:rgba(203,54,9,0.08); border:1px solid rgba(203,54,9,0.18);
...
.CP-pay-amt {
  ...
  color:#FF6B00; text-align:center; justify-self:center;
  background:linear-gradient(135deg,rgba(255,107,0,0.1),rgba(255,107,0,0.04));
  border:1px solid rgba(255,107,0,0.22); border-radius:7px;
...
.CP-pay-date {
  ...
  font-family:var(--ff-m); font-size: 7.5px; font-weight: 800; color:#6B4A2A;
  background:var(--surface,#FAF2E7); border:1px solid var(--border,#E7D7C1);
...
.CP-pay-more {
  ...
  background:rgba(255,107,0,0.045); border:none; border-top:1px solid var(--bd);
  transition:background .15s, color .15s;
}
.CP-pay-more:hover { background:rgba(255,107,0,0.1); color:#B8560A; }
.CP-pay-empty {
...
/* ── BUDGET HISTORY ── */
.CP-bgt-hist { margin:0 14px 14px; border-radius:10px; border:1.5px solid rgba(200,106,0,0.25); background:linear-gradient(135deg,#fffbf0,#fff); overflow:hidden; animation:bc-in 0.35s both; }
.CP-bgt-hist-hdr { display:flex; align-items:center; gap:7px; padding:9px 14px 8px; background:rgba(200,106,0,0.07); border-bottom:1px solid rgba(200,106,0,0.15); font-family:var(--ff-m); font-size: 8px; font-weight: 800; letter-spacing:1.5px; text-transform:uppercase; color:#C86A00; }
.CP-bgt-hist-count { background:rgba(200,106,0,0.15); color:#C86A00; font-family:var(--ff-m); font-size: 8px; font-weight: 800; padding:1px 7px; border-radius:100px; border:1px solid rgba(200,106,0,0.3); margin-left:auto; }
.CP-bgt-hist-list { padding:8px 12px 10px; display:flex; flex-direction:column; gap:6px; }
.CP-bgt-item { display:flex; align-items:center; gap:10px; padding:8px 10px; border-radius:8px; background:#fff; border:1px solid rgba(200,106,0,0.15); animation:bc-in 0.3s cubic-bezier(0.4,0,0.2,1) both; transition:box-shadow .2s, border-color .2s; }
.CP-bgt-item:hover { box-shadow:0 2px 10px rgba(200,106,0,0.1); border-color:rgba(200,106,0,0.3); }
.CP-bgt-item-icon { font-size: 14px; flex-shrink:0; }
.CP-bgt-item-body { flex:1; min-width:0; }
.CP-bgt-item-amt { font-family:var(--ff-m); font-size: 10.5px; font-weight: 800; color:#C86A00; }
...
.CP-modal-ttl  { font-family:var(--ff-b); font-size: 15px; font-style:normal; text-transform:uppercase; font-weight: 800; letter-spacing:0.5px; color:#FF6B00; line-height:1.1; }
.CP-modal-sub  { font-family:var(--ff-m); font-size: 9px; font-weight: 800; color:#FF6B00; margin-top:3px; }
...
/* FORM DIALOG SYSTEM — Add Client / Collect Payment / Add Budget / Add
   ...
   Credit Management's Add Ledger/Bill/Repayment dialogs (not a full page):
...
.CP-fp-modal {
  ...
  /* The modal card itself no longer scrolls (overflow:hidden + flex column)
     ...
     dialog (top strip + body), matching Credit Management's version. */
...
.CP-fp-close:hover { background: #fff7ed; border-color: #ff9f3a; color: #FF6B00; transform: rotate(90deg); box-shadow: 0 3px 10px rgba(203,54,9,0.14); }
...
.CP-cci-av { width:38px; height:38px; border-radius:var(--r2); flex-shrink:0; background:linear-gradient(135deg,var(--or,#FF6B00),var(--or2,#fe9307)); color:#fff; display:flex; align-items:center; justify-content:center; font-family:var(--ff-d); font-size: 14px; font-style:italic; }
...
.CP-pay-tbl thead tr { background:var(--surface-2,#F4E8D7); border-bottom:2px solid var(--ember,#FF6B00); }
.CP-pay-tbl th { padding:9px 12px; text-align:left; white-space:nowrap; font-family:var(--ff-m); font-size: 8px; font-weight: 800; text-transform:uppercase; letter-spacing:.1em; color:var(--text-3,#3C322A); background:transparent; border-bottom:none; border-right:1px solid var(--border,#e8e3d9); }
...
.CP-pay-tbl tbody tr:hover td { background:var(--or-t,rgba(255,107,0,.07)); }
.CP-pay-tbl tfoot td { padding:9px 12px; font-family:var(--ff-m); font-size: 9px; font-weight: 800; color:var(--or,#FF6B00); background:var(--or-t,rgba(255,107,0,.08)); border-top:2px solid var(--or-r,rgba(255,107,0,.3)); border-right:1px solid var(--bd); position:sticky; bottom:0; z-index:4; }
...
.CP-tx-page-btn {
  ...
  background:#fff; border:1.5px solid #fdc87a; color:#9a3412;
...
.CP-tx-page-btn:hover:not(:disabled) { background:#fff7ed; border-color:#ff9f3a; color:#FF6B00; transform:translateY(-1px); box-shadow:0 3px 8px rgba(203,54,9,0.14); }
...
.CP-tx-page-num { font-family:var(--ff-m); font-size: 9px; font-weight: 800; color:#FF6B00; min-width:20px; text-align:center; }
...
.CP-bud-tbl thead tr { background:var(--surface-2,#F4E8D7); border-bottom:2px solid var(--ember,#FF6B00); }
.CP-bud-tbl th { padding:9px 12px; font-family:var(--ff-m); font-size: 8px; font-weight: 800; text-transform:uppercase; letter-spacing:.1em; color:var(--text-3,#3C322A); background:transparent; border-bottom:none; border-right:1px solid var(--border,#e8e3d9); text-align:left; white-space:nowrap; }
...
.CP-bud-tbl tbody tr:hover td { background:var(--or-t,rgba(255,107,0,.07)); }
.CP-bud-tbl tfoot td { padding:9px 12px; font-family:var(--ff-m); font-size: 9px; font-weight: 800; color:var(--or,#FF6B00); background:var(--or-t,rgba(255,107,0,.08)); border-top:2px solid var(--or-r,rgba(255,107,0,.3)); border-right:1px solid var(--bd); position:sticky; bottom:0; }
...
.CP-bio-av  { width:26px; height:26px; border-radius:7px; background:linear-gradient(135deg,var(--or,#FF6B00),var(--or2,#fe9307)); color:#fff; display:flex; align-items:center; justify-content:center; font-family:var(--ff-d); font-size: 9.5px; font-style:italic; flex-shrink:0; }
...
@keyframes CP3d-glow { 0%,100%{box-shadow:0 8px 28px rgba(203,54,9,.1),0 2px 8px rgba(0,0,0,.05)} 50%{box-shadow:0 8px 36px rgba(203,54,9,.18),0 2px 12px rgba(0,0,0,.07)} }
...
/* Main panel */
.CP-c3d-panel { display:grid; grid-template-columns:260px 1fr; gap:0; background:#fff; border-radius:20px; overflow:hidden; position:relative; border:1.5px solid rgba(203,54,9,.18); box-shadow:0 8px 32px rgba(203,54,9,.12),0 2px 8px rgba(0,0,0,.06),inset 0 1px 0 rgba(255,255,255,.9); animation:CP3d-glow 3s ease-in-out infinite; }
@media(max-width:900px){ .CP-c3d-panel{ grid-template-columns:1fr; } }

/* Scan line */
.CP-c3d-scan { position:absolute; left:0; right:0; height:2px; background:linear-gradient(90deg,transparent,rgba(254,147,7,.35),transparent); animation:CP3d-scanline 4s linear infinite; pointer-events:none; z-index:10; }

/* Grid bg */
.CP-c3d-grid { position:absolute; inset:0; background-image:linear-gradient(rgba(203,54,9,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(203,54,9,.04) 1px,transparent 1px); background-size:32px 32px; pointer-events:none; }

/* LEFT — ring section */
.CP-c3d-left { position:relative; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:36px 28px; border-right:1.5px solid rgba(203,54,9,.12); background:linear-gradient(135deg,rgba(254,147,7,.07) 0%,rgba(203,54,9,.04) 60%,rgba(255,255,255,0) 100%); gap:16px; }
...
.CP-c3d-orb-dot { width:8px; height:8px; border-radius:50%; background:radial-gradient(circle,#fe9307,#FF6B00); box-shadow:0 0 8px rgba(254,147,7,.6); }
.CP-c3d-orb1 { animation:CP3d-orbit 4s linear infinite; }
.CP-c3d-orb2 { animation:CP3d-orbit2 6s linear infinite; }

/* Outer pulsing rings */
.CP-c3d-pulse-ring { position:absolute; inset:-12px; border-radius:50%; border:1.5px solid rgba(203,54,9,.18); animation:CP3d-pulse 2.5s ease-in-out infinite; pointer-events:none; }
.CP-c3d-pulse-ring2 { position:absolute; inset:-24px; border-radius:50%; border:1px solid rgba(254,147,7,.12); animation:CP3d-pulse2 3.2s ease-in-out infinite; pointer-events:none; }

/* SVG ring */
.CP-c3d-ring-svg { position:absolute; inset:0; width:100%; height:100%; transform:rotate(-90deg); filter:drop-shadow(0 0 6px rgba(254,147,7,.3)); }
.CP-c3d-ring-track { fill:none; stroke:rgba(203,54,9,.08); stroke-width:10; }
...
.CP-c3d-ring-spin { fill:none; stroke:rgba(254,147,7,.25); stroke-width:2; stroke-dasharray:8 16; animation:CP3d-spin 8s linear infinite; }
.CP-c3d-ring-spin2 { fill:none; stroke:rgba(203,54,9,.15); stroke-width:1.5; stroke-dasharray:4 20; animation:CP3d-spin-rev 12s linear infinite; }
...
.CP-c3d-pct { font-family:var(--ff-m,monospace); font-size: 33.5px; font-weight: 900; color:var(--ember,#FF6B00); letter-spacing:-2px; line-height:1; text-shadow:0 0 16px rgba(254,147,7,.25); }
.CP-c3d-pct-sym { font-size: 17.5px; vertical-align:super; font-weight: 800; color:#fe9307; }
.CP-c3d-pct-lbl { font-family:var(--ff-m,monospace); font-size: 8px; font-weight: 800; color:rgba(203,54,9,.5); text-transform:uppercase; letter-spacing:2px; }
...
.CP-c3d-bar-lbl-pct { font-family:var(--ff-m,monospace); font-size: 9px; font-weight: 800; color:var(--ember,#FF6B00); }
.CP-c3d-bar-track { height:14px; border-radius:99px; background:rgba(203,54,9,.07); border:1px solid rgba(203,54,9,.12); overflow:hidden; position:relative; }
.CP-c3d-bar-fill { height:100%; border-radius:99px; background:linear-gradient(90deg,#FF6B00 0%,#fe9307 40%,#ffb347 60%,#fe9307 80%,#FF6B00 100%); background-size:200% 100%; animation:CP3d-liquid 2s linear infinite; position:relative; overflow:hidden; min-width:6px; transition:width 1.2s cubic-bezier(.34,1.2,.64,1); box-shadow:0 0 10px rgba(254,147,7,.35),inset 0 1px 0 rgba(255,255,255,.4); }
...
.CP-c3d-card { background:#fff; border:1.5px solid rgba(203,54,9,.12); border-radius:14px; padding:16px 18px; position:relative; overflow:hidden; transform-style:preserve-3d; transition:transform .3s,box-shadow .3s; box-shadow:0 2px 12px rgba(203,54,9,.08); }
.CP-c3d-card:hover { transform:translateY(-4px) rotateX(6deg) scale(1.02); box-shadow:0 12px 28px rgba(203,54,9,.14); }
...
.CP-c3d-card::before { content:''; position:absolute; inset:0; background:linear-gradient(135deg,rgba(254,147,7,.05) 0%,transparent 60%); pointer-events:none; }
...
.CP-c3d-card-bar { height:3px; border-radius:99px; margin-top:10px; position:relative; overflow:hidden; background:rgba(203,54,9,.07); }
