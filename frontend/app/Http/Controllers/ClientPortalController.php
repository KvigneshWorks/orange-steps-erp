...
class ClientPortalController extends Controller
{
    ...
    {
        $clients = Client::with(['projects.payments'])
            ->orderBy('name')
            ->get()
            ->map(fn($c) => $this->formatClient($c));
    ...
    public function summary(): JsonResponse
    {
        ...
        foreach ($clients as $c) {
            foreach ($c->projects as $p) {
                ...
                $totalCollected  += (float) $p->payments->sum('amount');
                $totalAdditional += (float) $p->payments->sum('gst_amount');
                $totalProjects++;
            }
        }

        $totalBalance = $totalBudget - $totalCollected;
    ...
    private function formatPayment(ClientPayment $pay): array
    {
        return [
            ...
            'notes'            => $pay->notes,
            'next_due_date'    => $pay->next_due_date?->format('d M Y'),
